...
import BgCanvas from '../assets/asd.png'
import '../styles/Home.css'

function Home() {
    return (
        <div className="w-full h-full relative -z-20 bg-orang">
            <img src={BgCanvas} alt={BgCanvas} className="w-[100%] h-[700px]"/>
            <div className="absolute top-1/2 left-1/2 transform translate-x-[-50%] translate-y-[-50%] flex flex-col items-center justify-center p-10 ">
                ...
                <div className="content-right flex gap-10 max-sm:w-full max-sm:flex max-sm:flex-col max-sm:items-center max-sm:gap-4 ">
                    {ListBox.map(listBox => (
                        <div className="list-detail w-[320px] flex flex-col items-center p-8 rounded-2xl transition duration-200 ease-in-out bg-[#e1e1e1] hover:bg-black ">
                            <img src={listBox.image} alt={listBox.image} className="h-[120px] max-sm:h-[80px]" />
                            <h2 className="text-[#5E5DF0] text-xl font-medium p-4">{listBox.tittle}</h2>
